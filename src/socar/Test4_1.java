...
public class Test4_1 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Result 3: " + new Test4_1().solution(tickets3, roll3, shop3, money3));
        System.out.println("Result 4: " + new Test4_1().solution(tickets4, roll4, shop4, money4));
    }
    ...
    public int solution(String[] tickets, int roll, String[][] shop, int money) {
        ...
        // 최초 상점에서의 구매 시도
        maxGoldTickets = Math.max(maxGoldTickets, tryToBuyTickets(ticketPrices, shop[0], currentMoney, 1));

        for (int i = 0; i < shop.length; i++) {
            if (currentMoney >= roll) {
                currentMoney -= roll; // 상점 새로고침 비용 지불
                int goldTicketsThisRound = tryToBuyTickets(ticketPrices, shop[i], currentMoney, 1);
                maxGoldTickets = Math.max(maxGoldTickets, goldTicketsThisRound);
            }
        }
    ...
    private int tryToBuyTickets(Map<String, Integer> ticketPrices, String[] shopTickets, int money, int goldTickets) {
        Map<String, Integer> ticketCounts = new HashMap<>();
        for (String ticket : shopTickets) {
        ...
        for (Map.Entry<String, Integer> entry : ticketCounts.entrySet()) {
            goldTickets += entry.getValue() / 3; // 동일한 티켓 3개를 황금 티켓 1개로 교환
        }
