...
public class Test1_1 {
    public int[] solution(int n, int k, int[][] roads) {
        ...
        for (int[] road : roads) {
            graph.computeIfAbsent(road[0], x -> new ArrayList<>()).add(new int[]{road[1], road[2]});
            graph.computeIfAbsent(road[1], x -> new ArrayList<>()).add(new int[]{road[0], road[2]});
        }
    ...
    private void dfs(Map<Integer, List<int[]>> graph, int current, int k, Set<Integer> result, int currentTime) {
        if (currentTime == k) {
            result.add(current);
            return;
        }
        ...
        for (int[] neighbor : graph.get(current)) {
            dfs(graph, neighbor[0], k, result, currentTime + neighbor[1]);
        }
    }

    public static void main(String[] args) {
        Test1_1 solution = new Test1_1();
