...
입출력 예 #2
처음 자판기에서 내려오는 무게가 8인 상품이 통로에 걸립니다. 이후 다섯 번째까지는 위에 쌓인 상품의 무게 함이 8 미만이므로 상품이 빠져나오지 않습니다.
마지막으로 무게가 2인 상품이 내려오면 위에 쌓인 상품의 무게 합이 9가 되어 무게가 8인 상품이 자판기에서 빠져나옵니다.
다음으로 통로에 걸리는 상품들보다 위에 쌓인 상품의 무게 합이 더 커 마지막 무게가 2인 상품을 제외하고 전부 자판기 밖으로 빠져나옵니다.
...
public class Test4_1 {
    ...
    public static int solution(int[] products, int w) {
        Deque<Integer> chute = new ArrayDeque<>();
        int maxReleased = 0;

        for (int product : products) {
            if (product >= w) {
                // w 이상이면 바로 배출
                maxReleased = Math.max(maxReleased, product);
                int released = tryRelease(chute, product);
                maxReleased = Math.max(maxReleased, released);
            } else {
                // w 미만이면 걸림
                chute.addLast(product);
                int released = tryRelease(chute, 0);
                maxReleased = Math.max(maxReleased, released);
            }
        }
    ...
    private static int tryRelease(Deque<Integer> chute, int topWeight) {
        if (chute.isEmpty()) return 0;

        List<Integer> list = new ArrayList<>(chute);
        int n = list.size();

        // 아래에서부터 위에 쌓인 무게 누적
        for (int i = 0; i < n; i++) {
            int cur = list.get(i);
            int aboveWeight = topWeight;
            for (int j = i + 1; j < n; j++) {
                aboveWeight += list.get(j);
            }
    ...
    public static void main(String[] args) {
        System.out.println("✅ 내가 진짜 실행되고 있는 코드다!");
        System.out.println("Expected: 13 → Actual: " + solution(
                new int[]{5, 2, 3, 10, 5}, 10));
        System.out.println("Expected: 15 → Actual: " + solution(
                new int[]{8, 2, 2, 2, 1, 2}, 9));
    }
}
