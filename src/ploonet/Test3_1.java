...
public class Test3_1 {
    public int[] solution(String[] approved, String[] spams, String[] calls, int k) {
        HashSet<String> approvedSet = new HashSet<>(Arrays.asList(approved));
        HashSet<String> spamSet = new HashSet<>(Arrays.asList(spams));
        HashMap<String, Integer> callCounts = new HashMap<>();

        int[] results = new int[calls.length];
        ...
        for (int i = 0; i < calls.length; i++) {
            ...
            if (approvedSet.contains(call)) {
                results[i] = 0;
            } else if (spamSet.contains(call)) {
                results[i] = 1;
            } else {
                int count = callCounts.getOrDefault(call, 0) + 1;
                callCounts.put(call, count);
                if (count <= k){
                    results[i] = 1;
                }
            }
